"""
Gemini API 消息格式化属性测试

使用 Hypothesis 进行基于属性的测试，验证 Gemini API 消息格式化的通用正确性属性
"""
import pytest
...
class TestGeminiAPIMessageFormattingProperties:
    """Gemini API 消息格式化属性测试类"""
    
    def setup_method(self):
        """每个测试方法前的设置"""
        # 使用虚拟 API key 进行测试（不会实际调用 API）
        self.executor = GeminiAPIExecutor(api_key="test_key")
    
    # Feature: feishu-ai-bot, Property 41: Gemini API 消息格式化
    ...
    def test_format_messages_with_no_history(self, user_prompt):
        ...
        assert "parts" in message, "Message should have 'parts' field"
        
        # The message should be a user message with the prompt
        assert message["role"] == "user", \
            f"Expected role 'user', got '{message['role']}'"
        assert isinstance(message["parts"], list), "parts should be a list"
        assert len(message["parts"]) == 1, "parts should contain exactly one element"
        assert message["parts"][0] == user_prompt, \
            f"Expected parts[0] '{user_prompt}', got '{message['parts'][0]}'"
    ...
    def test_format_messages_with_history(self, user_prompt, history_messages):
        """
        Property 41: Gemini API 消息格式化 - 包含对话历史
        ...
        for i, (expected_role, expected_content) in enumerate(history_messages):
            ...
            assert messages[i]["role"] == expected_gemini_role, \
                f"Message {i}: expected role '{expected_gemini_role}', got '{messages[i]['role']}'"
            assert messages[i]["parts"][0] == expected_content, \
                f"Message {i}: expected content '{expected_content}', got '{messages[i]['parts'][0]}'"
        
        # Verify the last message is the current user prompt
        last_message = messages[-1]
        assert last_message["role"] == "user", \
            f"Last message should be 'user', got '{last_message['role']}'"
        assert last_message["parts"][0] == user_prompt, \
            f"Last message content should be '{user_prompt}', got '{last_message['parts'][0]}'"
    ...
    def test_format_messages_preserves_order(self, user_prompt, num_history_messages):
        """
        Property 41: Gemini API 消息格式化 - 保持消息顺序
        ...
        for i, hist_msg in enumerate(conversation_history):
            expected_role = "user" if hist_msg.role == "user" else "model"
            assert messages[i]["role"] == expected_role, \
                f"Message {i}: role order not preserved"
            assert messages[i]["parts"][0] == hist_msg.content, \
    ...
    def test_format_messages_converts_assistant_to_model(self, user_prompt, history_messages):
        """
        Property 41: Gemini API 消息格式化 - 将 assistant 转换为 model
        
        For any conversation history containing "assistant" role,
        format_messages should convert it to "model" role as required by Gemini API.
        ...
        # All messages should have valid Gemini roles (user or model, never assistant)
        valid_roles = {"user", "model"}
        for i, message in enumerate(messages):
            assert message["role"] in valid_roles, \
                f"Message {i}: invalid role '{message['role']}', expected one of {valid_roles}"
            # Ensure no "assistant" role appears
            assert message["role"] != "assistant", \
                f"Message {i}: 'assistant' role should be converted to 'model'"
    ...
    def test_format_messages_always_ends_with_user(self, user_prompt, history_messages):
        """
        Property 41: Gemini API 消息格式化 - 总是以用户消息结束
        ...
        assert messages[-1]["parts"][0] == user_prompt, \
    ...
    def test_format_messages_structure_consistency(self, user_prompt, history_messages):
        """
        Property 41: Gemini API 消息格式化 - 消息结构一致性
        
        For any formatted messages, each message should have
        the same structure: a dict with "role" and "parts" keys,
        where "parts" is a list containing the content.
        ...
        for i, message in enumerate(messages):
            ...
            assert set(message.keys()) == {"role", "parts"}, \
                f"Message {i}: should have exactly 'role' and 'parts' keys, got {message.keys()}"
            assert isinstance(message["role"], str), \
                f"Message {i}: role should be string"
            assert isinstance(message["parts"], list), \
                f"Message {i}: parts should be a list"
            assert len(message["parts"]) == 1, \
                f"Message {i}: parts should contain exactly one element"
            assert isinstance(message["parts"][0], str), \
                f"Message {i}: parts[0] should be string"
    ...
    def test_format_messages_handles_long_history(self, user_prompt, num_messages):
        """
        Property 41: Gemini API 消息格式化 - 处理长对话历史
        ...
        for i in range(num_messages):
            assert messages[i]["parts"][0] == f"Message number {i} with some content", \
    ...
    def test_format_messages_handles_special_characters(self, user_prompt, special_chars):
        """
        Property 41: Gemini API 消息格式化 - 处理特殊字符
        ...
        assert messages[0]["parts"][0] == special_chars, \
            "Special characters in history should be preserved"
        assert messages[1]["parts"][0] == f"Response to: {special_chars}", \
            "Special characters in history response should be preserved"
        assert messages[2]["parts"][0] == user_prompt, \
    ...
    def test_format_messages_empty_history_equivalent_to_none(self, user_prompt):
        """
        Property 41: Gemini API 消息格式化 - 空历史等同于无历史
    ...
    def test_format_messages_does_not_modify_input(self, user_prompt, history_messages):
        """
        Property 41: Gemini API 消息格式化 - 不修改输入
    ...
    def test_format_messages_idempotent_for_same_input(self, user_prompts):
        """
        Property 41: Gemini API 消息格式化 - 相同输入产生相同输出
    ...
    def test_format_messages_role_conversion_consistency(self, user_prompt, assistant_messages):
        """
        Property 41: Gemini API 消息格式化 - 角色转换一致性
        
        For any conversation history with assistant messages,
        all assistant roles should be consistently converted to model roles.
        
        Validates: Requirements 15.3
        """
        # Create history with only assistant messages
        conversation_history = [
            Message(role="assistant", content=content, timestamp=1000000 + i)
        ...
        for i in range(len(assistant_messages)):
            assert messages[i]["role"] == "model", \
                f"Message {i}: assistant role should be converted to 'model', got '{messages[i]['role']}'"
