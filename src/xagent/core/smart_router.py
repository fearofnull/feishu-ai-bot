...
class SmartRouter:
    ...
    def route(self, parsed_command: ParsedCommand) -> AIExecutor:
        ...
        # 没有显式前缀：默认使用 Agent
        logger.info("[ROUTING] No explicit prefix, using Agent")
        logger.debug(f"[ROUTING] Message: '{message_preview}'")
        
        # 检查Agent功能是否启用
        if hasattr(self.bot_config, 'agent_enabled') and not self.bot_config.agent_enabled:
            logger.error("[ROUTING] Agent functionality is disabled")
            raise ExecutorNotAvailableError(
                "agent",
                "agent",
                "Agent 功能已被管理员禁用。\n\n请使用CLI命令前缀（如 @claude-cli, @gemini-cli）。\n\n访问 http://localhost:8080 配置提供商。"
            )
        
        try:
            executor = self.get_executor("agent", "agent")
            logger.info("[ROUTING] ✅ Using agent/agent (default)")
            return executor
        except ExecutorNotAvailableError as e:
            logger.error(f"[ROUTING] ❌ Agent executor not available: {e.reason}")
            raise ExecutorNotAvailableError(
                "agent",
                "agent",
                f"Agent 执行器不可用: {e.reason}\n\n请检查配置。\n\n或使用CLI命令前缀（如 @claude-cli, @gemini-cli）。\n\n访问 http://localhost:8080 配置提供商。"
            )
