...
```bash
python -m feishu_bot.web_admin.server --disable-rate-limiting
```
...
```python
from feishu_bot.web_admin.rate_limiter import create_limiter
...
```
[2024-01-01 12:00:00] [WARNING] [feishu_bot.web_admin.rate_limiter] Rate limit exceeded for 192.168.1.100 on /api/auth/login
```
...
To customize rate limits, modify the `configure_rate_limits()` function in `feishu_bot/web_admin/rate_limiter.py`:
